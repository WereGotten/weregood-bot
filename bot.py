...
# ========== ОПТИМИЗАЦИЯ: ПАКЕТНАЯ ОБРАБОТКА КЛИКОВ ==========
class ClickBatcher:
    def __init__(self):
        self.batch = defaultdict(lambda: {"clicks": 0, "wg": 0.0})
    ...
    def add_click(self, user_id, wg_earned):
        with self.lock:
            self.batch[user_id]["clicks"] += 1
            self.batch[user_id]["wg"] += wg_earned
    ...
    def _flush_batch(self):
        ...
        try:
            with db.get_cursor() as cursor:
                for user_id, data in batch_to_process.items():
                    cursor.execute('''
                        UPDATE users 
                        SET total_clicks = total_clicks + ?, 
                            wg = wg + ? 
                        WHERE user_id = ?
                    ''', (data["clicks"], data["wg"], user_id))
            if random.random() < 0.1:
                logger.info(f"📊 Пакетная обработка: {len(batch_to_process)} пользователей")
        except Exception as e:
            logger.error(f"Ошибка в пакетной обработке: {e}")


click_batcher = ClickBatcher()


# ========== ОПТИМИЗАЦИЯ: КЭШ ПОЛЬЗОВАТЕЛЕЙ ==========
class UserCache:
    ...
    def get(self, user_id):
        with self.lock:
            ...
            if user_id in self.cache:
                if now - self.cache_time.get(user_id, 0) < self.ttl:
    ...
    def set(self, user_id, user_data):
        with self.lock:
            self.cache[user_id] = user_data.copy() if user_data else {}
            self.cache_time[user_id] = time.time()
            if len(self.cache) > self.max_size:
                items = sorted(self.cache_time.items(), key=lambda x: x[1])
                for uid, _ in items[:self.max_size // 10]:
                    if uid in self.cache:
                        del self.cache[uid]
                    if uid in self.cache_time:
                        del self.cache_time[uid]
...
user_cache = UserCache(ttl=60, max_size=5000)


# ========== ОПТИМИЗАЦИЯ: ПУЛ СОЕДИНЕНИЙ SQLITE ==========
...
db_pool = ConnectionPool(DATABASE_PATH, pool_size=10)
...
def add_log(action, user_id, username, old_value=None, new_value=None, currency="", details=""):
    ...
    with db.get_cursor() as cursor:
        cursor.execute(
            ...
            (timestamp, log_message[:500], user_id, (username or '')[:100], (details or '')[:200]))
...
def add_admin_log(action, admin_id, admin_name, target_id=None, target_name=None, details=""):
    ...
    with db.get_cursor() as cursor:
        cursor.execute(
            ...
            (timestamp, log_msg[:500], admin_id, admin_name[:100], (details or '')[:200]))
...
def update_achievement_progress(user_id, achievement_name, increment=1, set_value=None):
    with db.get_cursor() as cursor:
        ...
        if not ach:
            return False
        cursor.execute('''
            INSERT INTO user_achievements (user_id, achievement_id, current_count, is_completed)
            VALUES (?, ?, ?, 0)
            ON CONFLICT(user_id, achievement_id) DO UPDATE SET
            current_count = current_count + ?
        ''', (user_id, ach['id'], increment, increment))
    return True
...
def get_total_earning(upgrade_counts):
    UPGRADE_CONFIG = {1: {"bonus": 0.01}, 2: {"bonus": 0.03}, 3: {"bonus": 0.07}}
    base = 0.01
    total_bonus = 0
...
def init_db():
    with db.get_cursor() as cursor:
        ...
        cursor.execute('CREATE INDEX IF NOT EXISTS idx_users_clicks ON users(total_clicks DESC)')
...
def add_referral_earning(referrer_id, referred_id, spent_lp):
    ...
    with db.get_cursor() as cursor:
        ...
        if row:
            ...
            add_log(f"👥 Получил 5% от трат реферала (+{earning:.2f} LP)", referrer_id, "", old_value=old_lp,
                    new_value=new_lp, currency="lp")
        ...
        if lottery_tickets:
            ...
            try:
                socketio.emit('draw_completed', {
                    ...
                    'end_time': (draw_time + datetime.timedelta(seconds=10800)).isoformat()
                })
            except:
                pass
...
def distribute_prizes():
    ...
    for winner in winners:
        if not winner["ticket"].get("reward_claimed", False):
            ...
            add_log(f"🏆 ПОБЕДА в лотерее! +{prize_per_winner} USDT", winner["user_id"], "", currency="usdt")
...
def grant_energy_upgrade(user_id):
    with db.get_cursor() as cursor:
        ...
        invalidate_cache(user_id)
        add_admin_log(f"⭐ Купил энергетический усилитель за Stars | +40 макс. энергии, +50 LP",
...
def get_logs(log_type='all', limit=100, offset=0, date=None, action_filter=None, user_id_filter=None):
    with db.get_cursor() as cursor:
        ...
        logs = []
        for row in rows:
            logs.append({"id": row['id'], "timestamp": row['timestamp'], "action": row['action'][:500],
                         "user_id": row['user_id'], "username": row['username'][:100] if row['username'] else '',
                         "details": row['details'][:200] if row['details'] else '', "type": row['log_type']})
        return logs, total
...
def api_achievements_list():
    ...
    return jsonify({"success": True, "achievements": achievements, "completed_count": completed_count, "total_count": len(all_achievements)})
...
def health_check():
    ...
    return jsonify({"status": "ok", "online_users": online_count, "threads": threading.active_count(),
                    "db_size": os.path.getsize(DATABASE_PATH) if os.path.exists(DATABASE_PATH) else 0, "timestamp": time.time()})

@app.route('/tonconnect-manifest.json', methods=['GET'])
def serve_manifest():
    current_origin = f"{request.scheme}://{request.host}"
    manifest = {"url": current_origin, "name": "WereGood Game", "iconUrl": f"{current_origin}/static/coin.png",
...
def api_click():
    ...
    click_batcher.add_click(user_id, earning)
    user["wg"] = new_wg
    ...
    return jsonify({"energy": new_energy, "wg": new_wg, "lp": new_lp_value, "total_clicks": user["total_clicks"], "earned": earning, "lp_reward": lp_reward, "earning_per_click": earning})
        ...
        if existing:
            ...
            if referral_code:
                ...
                if referrer_row:
                    ...
                    cursor.execute('INSERT INTO referrals (referrer_id, referred_id, username, first_name, total_spent_lp) VALUES (?, ?, ?, ?, 0)',
                    ...
                    update_achievement_progress(referrer_id, 'social', 1)
            cursor.execute('''
                INSERT INTO users (user_id, wg, lp, energy, last_energy_update, tickets, total_clicks, upgrade_counts, 
...
def api_watch_ad():
    ...
    add_log(f"🎬 Просмотрел рекламу (+150 энергии)", user_id, user['username'], old_value=old_energy, new_value=new_energy, currency="energy")
...
def api_daily_leaderboard():
    with db.get_cursor() as cursor:
        today = datetime.datetime.now().strftime("%Y-%m-%d")
        cursor.execute('SELECT user_id, total_clicks, username, first_name, avatar_url, role, settings FROM users ORDER BY total_clicks DESC LIMIT 50')
        rows = cursor.fetchall()
        ...
        for i, row in enumerate(rows, 1):
            ...
            if hide_from_top:
                ...
                display_name = f"@{row['username']}" if row['username'] else (row['first_name'] or f"Player_{row['user_id']}")
                avatar = row['avatar_url'] or '👤'
            result.append({"rank": i, "user_id": row['user_id'], "username": display_name,
                          "total_clicks": row['total_clicks'], "avatar": avatar,
                          "role": row['role'] or 'player', "reward": rewards.get(i, "")})
    return jsonify(result)
        ...
        if row:
            ...
            if hide_from_top:
                ...
                if row['username'] and row['username'] != '':
                    ...
                    display_name = f"Player_{user_id}"
            return jsonify({"wg": row['wg'], "lp": row['lp'], "total_clicks": row['total_clicks'], "likes": row['likes'] or 0,
                           "dislikes": row['dislikes'] or 0, "username": display_name,
                           "avatar_url": row['avatar_url'] or "👤", "usdt": row['usdt'] if 'usdt' in row.keys() else 0,
                           "wins": row['wins'] if 'wins' in row.keys() else 0, "role": row['role'] if 'role' in row.keys() else 'player',
                           "stars": row['stars'] if 'stars' in row.keys() else 0, "max_energy": row['max_energy'] if 'max_energy' in row.keys() else 500,
                           "energy_upgrades": row['energy_upgrades'] if 'energy_upgrades' in row.keys() else 0, "hide_from_top": hide_from_top})
...
def api_get_referrals():
    ...
    with db.get_cursor() as cursor:
        ...
        for row in rows:
            ...
            total_earned += earned
            referrals.append({"username": name, "date": row['created_at'], "spent_lp": row['total_spent_lp'] or 0, "earned": round(earned, 2)})
...
def api_vote():
    ...
    with db.get_cursor() as cursor:
        ...
        cursor.execute('INSERT INTO votes (voter_id, target_id, vote_type, last_vote_time) VALUES (?, ?, ?, ?) ON CONFLICT(voter_id, target_id) DO UPDATE SET vote_type=?, last_vote_time=?',
                       (voter_id, target_id, vote_type, datetime.datetime.now().isoformat(), vote_type, datetime.datetime.now().isoformat()))
        if vote_type == 'like':
...
def api_daily_status():
    ...
    with db.get_cursor() as cursor:
        ...
        return jsonify({"current_day": current_day, "can_claim": can_claim, "next_claim_time": next_claim_time.isoformat() if next_claim_time else None, "recovered_count": recovered_count, "lost_streak": lost_streak})
...
def api_claim_daily():
    ...
    if not is_valid:
        return jsonify({"success": False, "msg": "Invalid user_id"}), 400
    DAILY_REWARDS = {1: {"wg": 15}, 2: {"wg": 50}, 3: {"energy_limit": 10}, 4: {"lp": 10}, 5: {"energy_limit": 15}, 6: {"wg": 150}, 7: {"lp": 20}}
    with db.get_cursor() as cursor:
        ...
        cursor.execute('UPDATE daily_rewards SET current_day = ?, last_claim_date = ?, recovered_count = ? WHERE user_id = ?', (new_day, now.isoformat(), row['recovered_count'] or 0, user_id))
...
def api_recover_daily():
    ...
    with db.get_cursor() as cursor:
        ...
        return jsonify({"success": True, "msg": f"Серия восстановлена! Вы можете забрать награду за {current_day} день!", "current_day": current_day})
...
def api_get_tasks():
    ...
    if not is_valid:
        return jsonify({'success': False, 'error': 'Invalid user_id'}), 400
    with db.get_cursor() as cursor:
        cursor.execute('SELECT t.*, CASE WHEN ut.id IS NOT NULL THEN 1 ELSE 0 END as is_completed FROM tasks t LEFT JOIN user_tasks ut ON t.id = ut.task_id AND ut.user_id = ? WHERE t.is_active = 1 ORDER BY is_completed ASC, t.created_at DESC', (user_id,))
        rows = cursor.fetchall()
        tasks = []
        for row in rows:
            tasks.append({'id': row['id'], 'title': row['title'], 'channel_link': row['channel_link'],
...
def api_check_task_subscription():
    ...
    with db.get_cursor() as cursor:
        ...
        try:
            ...
            if data_resp.get('ok'):
                status = data_resp.get('result', {}).get('status', '')
                if status in ['member', 'administrator', 'creator']:
                    ...
                    if task['reward_type'] == 'wg':
                        ...
                        update_energy_in_db(user_id, user, new_energy)
                        add_log(f"📋 Выполнил задание '{task['title']}' | +{task['reward_amount']} энергии", user_id, user['username'])
                    ...
                    return jsonify({'success': True, 'message': f'✅ Вы получили +{task["reward_amount"]} {task["reward_type"].upper()}!',
...
def api_get_available_prefixes():
    ...
    all_prefixes = {"player": {"name": "Игрок", "icon": "🎮", "desc": "Выдаётся абсолютно всем игрокам", "color": "player"},
                    "pioneer": {"name": "Первооткрыватель", "icon": "⭐", "desc": "За регистрацию в первый день", "color": "pioneer"},
...
def api_get_withdrawals():
    ...
    with db.get_cursor() as cursor:
        ...
        withdrawals = []
        for row in rows:
            withdrawals.append({"id": row['id'], "user_id": row['user_id'], "username": row['username'], "amount": row['amount'],
                               "address": row['address'], "network": row['network'], "status": row['status'],
                               "created_at": row['created_at'], "processed_at": row['processed_at']})
...
def api_activate_promo():
    ...
    with db.get_cursor() as cursor:
        ...
        cursor.execute("UPDATE promo_codes SET used_count = ? WHERE id = ?", (used_count + 1, promo['id']))
        cursor.execute('INSERT INTO promo_activations (promo_id, user_id) VALUES (?, ?)', (promo['id'], user_id))
        add_log(f"🎁 Активировал промокод {code} | +{promo['reward_amount']} {promo['reward_type'].upper()}", user_id, user['username'])
        return jsonify({"success": True, "message": f"Вы получили +{promo['reward_amount']} {promo['reward_type'].upper()}!",
...
def api_set_referral():
    ...
    with db.get_cursor() as cursor:
        ...
        if referrer and referrer['user_id'] != user_id:
            ...
            cursor.execute('INSERT INTO referrals (referrer_id, referred_id, username, first_name) SELECT ?, ?, username, first_name FROM users WHERE user_id = ?', (referrer['user_id'], user_id, user_id))
...
def api_ton_save_wallet():
    ...
    try:
        ...
        invalidate_cache(user_id)
        user = get_user(user_id)
        add_log(f"🔗 Привязал TON кошелёк: {wallet_address[:6]}...{wallet_address[-4:]}", user_id, user.get('username', 'Unknown'))
...
def api_ton_create_lp_boost_payment():
    ...
    payment_amount_nano = int(payment_amount_ton * 1e9)
    return jsonify({"success": True, "wallet_address": proj_wallet, "amount": payment_amount_ton, "amount_nano": payment_amount_nano, "comment": f"WereGood_LP:{user_id}"})
...
def check_lp_boost_payment():
    try:
        ...
        if confirmed:
            ...
            new_lp = old_lp + 50
            safe_update_user(user_id, lp=new_lp)
            add_admin_log(f"💎 Купил LP Бустер за TON ({expected_amount} TON) | +50 LP", user_id, user.get('username') or f"User_{user_id}", details=f"LP: {old_lp} → {new_lp}")
            return jsonify({'confirmed': True, 'tx_hash': tx_hash})
        return jsonify({'confirmed': False})
    except Exception as e:
        logger.error(f"Ошибка в check_lp_boost_payment: {e}")
        return jsonify({'confirmed': False, 'error': str(e)}), 500
...
def api_get_lp_boost_count():
    ...
    if not is_valid:
        return jsonify({"success": False, "error": "Invalid user_id"}), 400
    with db.get_cursor() as cursor:
        cursor.execute("SELECT COUNT(*) as count FROM successful_payments WHERE user_id = ? AND payload = 'lp_boost'", (user_id,))
        row = cursor.fetchone()
    ...
    with db.get_cursor() as cursor:
        ...
        for row in rows:
            ...
            if row['username'] and row['username'] != '':
                ...
                display_name = f"Player_{row['user_id']}"
            players.append({"user_id": row['user_id'], "username": display_name, "avatar_url": row['avatar_url'] or '', "time_ago": time_ago, "ticket_number": row['ticket_number'], "role": row['role'] if row['role'] else 'player'})
...
def api_reveal_all_tickets_fast():
    ...
    with lottery_lock:
        ...
        if revealed_count > 0:
            save_lottery()
            user = get_user(user_id)
            add_log(f"🔓 Открыл все клетки ({revealed_count} клеток, {tickets_revealed} билетов)", user_id, user['username'])
            return jsonify({"success": True, "msg": f"Открыто {revealed_count} клеток в {tickets_revealed} билетах!", "revealed": revealed_count})
        ...
        for ticket in lottery_tickets:
            if ticket.get("user_id") == user_id and ticket.get("number") == ticket_number:
                ...
                if revealed_count > 0:
                    save_lottery()
                    user = get_user(user_id)
                    add_log(f"🔓 Открыл все клетки билета #{ticket_number} ({revealed_count} клеток)", user_id, user['username'])
                    return jsonify({"success": True, "msg": f"Билет #{ticket_number} полностью открыт! ({revealed_count} клеток)"})
                else:
...
def api_admin_lottery_participants():
    ...
    for ticket in lottery_tickets:
        participants.append({"user_id": ticket.get('user_id'), "ticket_number": ticket.get('number'),
                            "purchase_number": ticket.get('purchase_number'), "revealed_count": sum(ticket.get('revealed', [])),
...
def api_admin_search_users():
    ...
    with db.get_cursor() as cursor:
        cursor.execute("SELECT user_id, username, first_name, role, wg, lp, usdt, wins, total_clicks, stars, max_energy, energy_upgrades, unlocked_prefixes FROM users WHERE user_id LIKE ? OR username LIKE ? OR first_name LIKE ? LIMIT 50",
                       (f'%{query}%', f'%{query}%', f'%{query}%'))
        rows = cursor.fetchall()
        users = []
        for row in rows:
            banned, _ = is_banned(row['user_id'])
            users.append({"user_id": row['user_id'], "username": row['username'] or row['first_name'] or str(row['user_id']),
                         "role": row['role'] or 'player', "wg": round(row['wg'] or 0, 2), "lp": int(row['lp'] or 0),
                         "usdt": round(row['usdt'] or 0, 2), "wins": int(row['wins'] or 0), "total_clicks": int(row['total_clicks'] or 0),
                         "stars": int(row['stars'] or 0), "max_energy": int(row['max_energy'] or 500),
                         "energy_upgrades": int(row['energy_upgrades'] or 0), "unlocked_prefixes": json.loads(row['unlocked_prefixes']) if row['unlocked_prefixes'] else ["player"],
                         "is_banned": banned})
...
def api_admin_chart_data():
    def get_stats_history(period, metric):
        now = datetime.datetime.now()
        data = []
        labels = []
        with db.get_cursor() as cursor:
...
def api_admin_create_task():
    ...
    admin_name = "Admin"
    with db.get_cursor() as cursor:
        cursor.execute('INSERT INTO tasks (title, channel_link, channel_username, channel_avatar, reward_amount, reward_type, daily_limit, total_limit, days_remaining) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)',
                       (title, channel_link, channel_username, channel_avatar, reward_amount, reward_type, daily_limit, total_limit, days_remaining))
...
def api_admin_update_task():
    ...
    admin_name = "Admin"
    with db.get_cursor() as cursor:
        cursor.execute('UPDATE tasks SET title = ?, channel_link = ?, channel_username = ?, channel_avatar = ?, reward_amount = ?, reward_type = ?, daily_limit = ?, total_limit = ?, days_remaining = ?, is_active = ? WHERE id = ?',
                       (title, channel_link, channel_username, channel_avatar, reward_amount, reward_type, daily_limit, total_limit, days_remaining, is_active, task_id))
...
def api_admin_get_promo_codes():
    with db.get_cursor() as cursor:
        cursor.execute('SELECT p.*, COUNT(a.id) as used_count, GROUP_CONCAT(a.user_id || ":" || a.activated_at) as activations FROM promo_codes p LEFT JOIN promo_activations a ON p.id = a.promo_id GROUP BY p.id ORDER BY p.created_at DESC')
        rows = cursor.fetchall()
        promos = []
        for row in rows:
            activations_list = []
...
def api_admin_create_promo():
    ...
    with db.get_cursor() as cursor:
        cursor.execute('INSERT INTO promo_codes (code, reward_type, reward_amount, max_uses, password, created_by) VALUES (?, ?, ?, ?, ?, ?)',
                       (code, reward_type, reward_amount, max_uses, password, admin_id))
        promo_id = cursor.lastrowid
    add_admin_log(f"🎫 Создал промокод {code} | {reward_type}: {reward_amount} | Макс: {max_uses} | Пароль: {'Да' if password else 'Нет'}", admin_id, admin_name, details=f"ID: {promo_id}")
    promo_url = f"https://t.me/WereGooodbot/WereGood?startapp=claim_{code}"
    web_url = f"https://weregood.ru/claim?code={code}"
    return jsonify({"success": True, "promo_id": promo_id, "code": code, "promo_url": promo_url, "web_url": web_url})
...
def handle_telegram_updates():
    last_update_id = 0
    verify_ssl = not DEBUG_MODE
    def send_telegram_message_simple(chat_id, text, reply_markup=None):
        url = f"https://api.telegram.org/bot{TELEGRAM_TOKEN}/sendMessage"
        try:
            data = {"chat_id": chat_id, "text": text, "parse_mode": "Markdown"}
            if reply_markup:
            ...
            if updates.get("ok"):
                for update in updates.get("result", []):
                    ...
                    if "message" in update and "text" in update["message"]:
                        ...
                        username = update["message"]["chat"].get("username", "")[:50]
                        first_name = update["message"]["chat"].get("first_name", "")[:50]
                        if text.startswith("/start"):
                                ...
                                if not existing:
                                    ...
                                    if ref_code:
                                        ...
                                        if referrer_row:
                                            ...
                                            send_telegram_message_simple(referrer_id, f"🎉 Новый реферал! {first_name or username} присоединился по вашей ссылке!")
                            ...
                            if chat_id in ADMIN_IDS:
                                ...
                                send_telegram_message_simple(chat_id, "👑 Админ-панель WereGood\n\n• 📊 Статистика\n• 💰 Выдача валюты\n• 🎲 Управление лотереей\n• 👑 Управление префиксами\n• 💸 Заявки на вывод\n• 🎫 Промокоды\n\n⬇️ Нажми на кнопку", keyboard)
                            else:
                                send_telegram_message_simple(chat_id, "⛔ У вас нет доступа к админ-панели")
                        ...
                        try:
                            ...
                            stars_amount = payment_info.get('total_amount', 100) // 100
                            if not user_id:
                                continue
                            with db.get_cursor() as cursor:
                                ...
                                if cursor.fetchone():
                                    continue
                                cursor.execute("INSERT INTO successful_payments (user_id, telegram_payment_charge_id, payload, amount) VALUES (?, ?, ?, ?)",
                            ...
                            if payment_type == 'lp_boost':
                                ...
                                new_lp = old_lp + 50
                                safe_update_user(user_id, lp=new_lp)
                                send_telegram_message_simple(chat_id, f"✅ LP Бустер активирован! +50 LP на баланс!")
                                ...
                                success, message, upgrade_data = grant_energy_upgrade(user_id)
                                if success:
                                    send_telegram_message_simple(chat_id, f"✅ {message}\n✨ Макс. энергия: {upgrade_data['max_energy']}\n💎 LP: {upgrade_data['lp']}")
                                    today = datetime.datetime.now().strftime("%Y-%m-%d")
                                    update_stats_history(today, stars=stars_amount)
                                else:
                                    send_telegram_message_simple(chat_id, f"❌ {message}")
...
def calculate_daily_top():
    with db.get_cursor() as cursor:
        today = datetime.datetime.now().strftime("%Y-%m-%d")
        cursor.execute('SELECT user_id, total_clicks, username, first_name, role FROM users ORDER BY total_clicks DESC LIMIT 5')
        rows = cursor.fetchall()
        rewards = {1: {"lp": 70, "wg": 200}, 2: {"lp": 50, "wg": 150}, 3: {"lp": 35, "wg": 120}, 4: {"lp": 25, "wg": 100}, 5: {"lp": 15, "wg": 75}}
        for i, row in enumerate(rows, 1):
            if i in rewards:
                ...
                add_log(f"🏆 ТОП ДНЯ #{i} место | +{rewards[i]['lp']} LP, +{rewards[i]['wg']} WG", row['user_id'], row['username'] or row['first_name'] or str(row['user_id']))
                send_telegram_message(row['user_id'], f"🏆 ПОЗДРАВЛЯЕМ!\n\nВы заняли #{i} место в ТОПЕ ДНЯ!\n\nНаграда:\n💎 +{rewards[i]['lp']} LP\n💰 +{rewards[i]['wg']} WG\n\nПродолжайте кликать! 🎉")
...
if __name__ == '__main__':
    ...
    print("=" * 60)
    print("✅ ВКЛЮЧЕННЫЕ ОПТИМИЗАЦИИ:")
    print("   • Пакетная обработка кликов (до 1000+ кликов/сек)")
    print("   • Кэширование пользователей с TTL")
    print("   • Пул соединений SQLite (10 коннектов)")
    print("   • Оптимизированные индексы БД")
    print("   • Rate limiting для защиты от DDoS")
    print("=" * 60)
    print(f"🌐 Игра: http://0.0.0.0:5000")
    print(f"👑 Админ-панель: http://0.0.0.0:5000/admin?key={ADMIN_SECRET}")
    print("=" * 60)
    print("💡 РЕКОМЕНДАЦИИ ДЛЯ ЗАПУСКА В ПРОДАКШЕНЕ:")
    print("   gunicorn -k geventwebsocket.gunicorn.workers.GeventWebSocketWorker \\")
    print("            -w 4 --threads 4 --worker-connections 1000 \\")
    print("            -b 0.0.0.0:5000 bot:app")
    print("=" * 60)
