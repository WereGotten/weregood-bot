...
SLOT_CONFIG = {
    ...
    'spin_cost': 10,  # WG Coin
    'secret_user_id': 5264622363  # ТВОЙ ID
}
...
def update_fortune_achievements(user_id, bet_amount=None, is_win=False, is_new_round=True):
    ...
    def init_db():
        with db.get_cursor() as cursor:
            ...
            ''')
            cursor.execute('CREATE INDEX IF NOT EXISTS idx_slot_user_id ON slot_history (user_id)')
...
# ========== ЛОГИКА СЛОТ-МАШИНЫ ==========
def get_weighted_symbols():
    """Возвращает список символов с учётом весов"""
    result = []
    for symbol, weight in SLOT_CONFIG['weights'].items():
        # Умножаем на 10 для точности
        result.extend([symbol] * int(weight * 10))
    return result


def generate_slot_result():
    """Генерирует случайный результат вращения"""
    weighted = get_weighted_symbols()

    symbols = []
    for _ in range(3):
        symbols.append(random.choice(weighted))

    combo = ''.join(symbols)
    multiplier = SLOT_CONFIG['payouts'].get(combo, 0)
    win_amount = multiplier * SLOT_CONFIG['spin_cost']
    is_jackpot = combo == '7️⃣7️⃣7️⃣'
...
def log_slot_spin(user_id, result):
    """Логирует вращение в базу данных"""
    try:
        with db.get_cursor() as cursor:
            cursor.execute('''
                INSERT INTO slot_history (user_id, symbol1, symbol2, symbol3, combo, win_amount, is_jackpot, created_at)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?)
            ''', (
                user_id,
                result['symbols'][0],
                ...
                result['combo'],
                result['win_amount'],
                1 if result['is_jackpot'] else 0,
                datetime.datetime.now().isoformat()
            ))
    except Exception as e:
        logger.error(f"❌ Ошибка логирования слота: {e}")
...
def api_slot_spin():
    """Обработчик вращения слот-машины"""
    data = request.json
    if not data:
        return jsonify({"success": False, "error": "No data"}), 400
    ...
    if user_id != SLOT_CONFIG['secret_user_id']:
        return jsonify({
            "success": False,
            "error": "⛔ Доступ запрещён. Эта игра только для разработчика."
        }), 403
    ...
    if banned:
        return jsonify({
            "success": False,
            "error": f"Вы забанены! Причина: {ban_info['reason']}"
        })
    ...
    # ===== ПРОВЕРКА БАЛАНСА =====
    if balance < SLOT_CONFIG['spin_cost']:
        return jsonify({
            "success": False,
            "error": f"❌ Недостаточно WG! Нужно {SLOT_CONFIG['spin_cost']} WG",
            "balance": balance
        })

    # ===== СПИСЫВАЕМ СТОИМОСТЬ =====
    old_wg = balance
    new_balance = balance - SLOT_CONFIG['spin_cost']
    safe_update_user(user_id, wg=new_balance)

    # ===== ГЕНЕРИРУЕМ РЕЗУЛЬТАТ =====
    result = generate_slot_result()

    # ===== НАЧИСЛЯЕМ ВЫИГРЫШ =====
    if result['win_amount'] > 0:
        new_balance += result['win_amount']
        safe_update_user(user_id, wg=new_balance)

        # Логируем выигрыш
        log_slot_spin(user_id, result)

        # Если джекпот - отправляем уведомление
        if result['is_jackpot']:
            try:
                send_telegram_message(
                    user_id,
                    f"🎰 **ДЖЕКПОТ!** 🎰\n\n"
                    f"💰 Вы выиграли {result['win_amount']} WG!\n"
                    f"🎯 Комбинация: {result['combo']}\n\n"
                    f"Поздравляем! 🎉"
                )
            except:
                pass
    ...
    # ===== ЛОГИРУЕМ ДЕЙСТВИЕ =====
    add_log(
        f"🎰 Слот-машина: {result['symbols']} -> {result['combo']} {'✅ ВЫИГРЫШ ' + str(result['win_amount']) + ' WG' if result['win_amount'] > 0 else '❌ ПРОИГРЫШ'}",
        user_id, user.get('username') or f"User_{user_id}"
    )
...
@app.route('/api/slot/stats', methods=['POST'])
def api_slot_stats():
    """Получение статистики игры в слоты"""
    data = request.json
    if not data:
        return jsonify({"success": False, "error": "No data"}), 400
    ...
    with db.get_cursor() as cursor:
        cursor.execute("SELECT COUNT(*) as total_spins FROM slot_history WHERE user_id = ?", (user_id,))
        total_spins = cursor.fetchone()[0] or 0

        cursor.execute("SELECT SUM(win_amount) as total_wins FROM slot_history WHERE user_id = ?", (user_id,))
        total_wins = cursor.fetchone()[0] or 0

        cursor.execute("SELECT COUNT(*) as jackpots FROM slot_history WHERE user_id = ? AND is_jackpot = 1", (user_id,))
        jackpots = cursor.fetchone()[0] or 0
...
@app.route('/api/slot/history', methods=['POST'])
def api_slot_history():
    """Получение истории вращений"""
    data = request.json
    if not data:
        return jsonify({"success": False, "error": "No data"}), 400
    ...
    with db.get_cursor() as cursor:
        ...
        for row in rows:
            history.append({
                "id": row['id'],
                "symbols": [row['symbol1'], row['symbol2'], row['symbol3']],
                "combo": row['combo'],
                "win_amount": row['win_amount'],
