...
def sync_fortune_from_db():
    ...
    with fortune_lock:
        with db.get_cursor() as cursor:
            ...
            end_time_str = round_row.get('end_time')
            ...
            for bet in bets:
                ...
                bet_data = {
                    ...
                    "net_amount": float(bet['net_amount']),
            ...
            print(f"✅ Синхронизирована Фортуна: раунд {round_id}")
...
def save_bet_to_db(round_id, user_id, team, amount, net_amount):
    with db.get_cursor() as cursor:
        cursor.execute('''
            SELECT id, amount, net_amount FROM fortune_active_bets 
            WHERE round_id = ? AND user_id = ? AND team = ?
        ''', (round_id, user_id, team))
        existing = cursor.fetchone()
        if existing:
            new_amount = existing['amount'] + amount
            new_net = existing['net_amount'] + net_amount
            cursor.execute('''
                UPDATE fortune_active_bets 
                SET amount = ?, net_amount = ?
                WHERE round_id = ? AND user_id = ? AND team = ?
            ''', (new_amount, new_net, round_id, user_id, team))
        else:
            cursor.execute('''
...
# Запускаем восстановление и таймер
...
# ========== ФОРТУНА API (ИСПРАВЛЕННЫЕ ЭНДПОИНТЫ) ==========
...
def api_fortune_status():
    ...
    with fortune_lock:
        ...
        yellow_bets_sorted = sorted(current_fortune_round['yellow_bets'], key=lambda x: x['net_amount'], reverse=True)[:5]
...
def api_fortune_bet():
    ...
    with fortune_lock:
        ...
        if existing_bet:
            ...
            save_bet_to_db(round_id, user_id, team, amount, net_amount)
            add_log(
    ...
    try:
        with fortune_lock:
            if current_fortune_round.get('round_id') and time.time() >= current_fortune_round.get('end_time', 0):
                with ending_round_lock:
                    if ending_round:
                        return jsonify({"success": False, "message": "Раунд уже завершается"})
                    ending_round = True
                try:
                    sync_fortune_from_db()
                    end_fortune_round_internal()
                ...
                return jsonify({"success": False, "message": "Раунд ещё не закончен", "time_left": max(0, int(current_fortune_round.get('end_time', 0) - time.time()))})
    except Exception as e:
...
def api_fortune_user_bet():
    ...
    with fortune_lock:
        ...
        for bet in current_fortune_round.get('yellow_bets', []):
            if bet.get('user_id') == user_id:
                return jsonify({"success": True, "bet": bet, "team": "yellow", "amount": bet.get('amount', 0), "net_amount": bet.get('net_amount', 0)})
        for bet in current_fortune_round.get('red_bets', []):
            if bet.get('user_id') == user_id:
                return jsonify({"success": True, "bet": bet, "team": "red", "amount": bet.get('amount', 0), "net_amount": bet.get('net_amount', 0)})
...
# ========== ОСТАЛЬНЫЕ API (СОХРАНЯЮТСЯ БЕЗ ИЗМЕНЕНИЙ) ==========
...
if __name__ == '__main__':
    ...
    print("   • КОМАНДНАЯ ФОРТУНА: мини-игра с комиссией 7% и шансами победы (ИСПРАВЛЕНА - ставки суммируются)")
    print("=" * 60)
